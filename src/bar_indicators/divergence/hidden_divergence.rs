    ...
    pub fn update_bar(&mut self, o: f64, h: f64, l: f64, c: f64, v: f64) -> f64 {
        ...
        if self.prices.len() >= self.lookback {
            ...
            if len >= self.lookback * 2 {
                ...
                // Filter for hidden divergence only
                self.value = match dtype {
                    DivergenceType::HiddenBullish => 1.0,
                    DivergenceType::HiddenBearish => -1.0,
                    _ => 0.0,
                };
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_hidden_divergence_creation() {
        let div = HiddenDivergence::new(14, 10);
        assert!(!div.is_ready());
        assert_eq!(div.value().main(), 0.0);
    }

    #[test]
    fn test_hidden_divergence_warmup() {
        let mut div = HiddenDivergence::new(14, 10);
        for i in 0..50 {
    ...
    #[test]
    fn test_hidden_divergence_values_range() {
        let mut div = HiddenDivergence::new(14, 10);
        for i in 0..60 {
    ...
    #[test]
    fn test_hidden_divergence_reset() {
        let mut div = HiddenDivergence::new(14, 10);
        for i in 0..50 {
