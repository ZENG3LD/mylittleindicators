use crate::bar_indicators::divergence::divergence::DivergenceDetector;
use crate::bar_indicators::momentum::stochastics::Stochastics;
use arrayvec::ArrayVec;
use crate::bar_indicators::indicator_value::IndicatorValue;

#[derive(Clone)]
pub struct StochasticDivergence {
    detector: DivergenceDetector,
    stoch: Stochastics,
    prices: ArrayVec<f64, 512>,
    stoch_values: ArrayVec<f64, 512>,
    lookback: usize,
    value: f64,
}

impl StochasticDivergence {
    pub fn new(period_k: usize, period_d: usize, lookback: usize) -> Self {
        Self {
            detector: DivergenceDetector::new(),
            stoch: Stochastics::new(period_k.max(1), period_d.max(1)),
            prices: ArrayVec::new(),
            stoch_values: ArrayVec::new(),
            lookback: lookback.max(5),
            value: 0.0,
        }
    }

    pub fn reset(&mut self) {
        self.detector = DivergenceDetector::new();
        self.stoch.reset();
        self.prices.clear();
        self.stoch_values.clear();
        self.value = 0.0;
    }

    pub fn is_ready(&self) -> bool {
        self.stoch.is_ready() && self.prices.len() >= self.lookback
    }

    pub fn value(&self) -> IndicatorValue {
        let signal = if self.value > 0.0 { 1i8 } else if self.value < 0.0 { -1i8 } else { 0i8 };
        IndicatorValue::Signal(signal)
    }

    pub fn update_bar(&mut self, _o: f64, h: f64, l: f64, c: f64, v: f64) -> f64 {
        let (k_value, _d_value) = self.stoch.update_bar(h, l, c, v);
        let price = c;

        if self.prices.len() >= 512 {
            self.prices.remove(0);
        }
        self.prices.push(price);

        if self.stoch_values.len() >= 512 {
            self.stoch_values.remove(0);
        }
        self.stoch_values.push(k_value);

        if self.prices.len() >= self.lookback {
            let len = self.prices.len();

            if len >= self.lookback * 2 {
                let price1 = self.prices[len - self.lookback];
                let price2 = self.prices[len - 1];
                let ind1 = self.stoch_values[len - self.lookback];
                let ind2 = self.stoch_values[len - 1];

                // Classic bullish divergence: price lower, Stochastic higher
                if price2 < price1 && ind2 > ind1 {
                    self.value = ((ind2 - ind1).abs() / 100.0).min(1.0);
                }
                // Classic bearish divergence: price higher, Stochastic lower
                else if price2 > price1 && ind2 < ind1 {
                    self.value = -((ind2 - ind1).abs() / 100.0).min(1.0);
                } else {
                    self.value = 0.0;
                }
            }
        }

        self.value
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_stochastic_divergence_creation() {
        let div = StochasticDivergence::new(14, 3, 10);
        assert!(!div.is_ready());
        assert_eq!(div.value().main(), 0.0);
    }

    #[test]
    fn test_stochastic_divergence_warmup() {
        let mut div = StochasticDivergence::new(14, 3, 10);
        for i in 0..50 {
            let price = 100.0 + (i as f64 * 0.1).sin() * 5.0;
            div.update_bar(price, price + 1.0, price - 1.0, price, 1000.0);
        }
        assert!(div.is_ready());
    }

    #[test]
    fn test_stochastic_divergence_values_finite() {
        let mut div = StochasticDivergence::new(14, 3, 10);
        for i in 0..60 {
            let price = 100.0 + (i as f64 * 0.2).sin() * 10.0;
            let value = div.update_bar(price, price + 1.0, price - 1.0, price, 1000.0);
            assert!(value.is_finite());
        }
    }

    #[test]
    fn test_stochastic_divergence_reset() {
        let mut div = StochasticDivergence::new(14, 3, 10);
        for i in 0..50 {
            div.update_bar(100.0 + i as f64, 105.0, 95.0, 101.0, 1000.0);
        }
        div.reset();
        assert!(!div.is_ready());
        assert_eq!(div.value().main(), 0.0);
    }
}
