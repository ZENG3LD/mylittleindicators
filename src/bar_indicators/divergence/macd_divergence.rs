...
impl MacdDivergence {
    pub fn new(fast: usize, slow: usize, lookback: usize) -> Self {
        Self {
    ...
    pub fn is_ready(&self) -> bool {
        self.macd.is_ready() && self.prices.len() >= self.lookback
    }
    ...
    pub fn update_bar(&mut self, o: f64, h: f64, l: f64, c: f64, v: f64) -> f64 {
        ...
        if self.prices.len() >= self.lookback {
            ...
            if len >= self.lookback * 2 {
                ...
                let ind1 = self.macd_values[len - self.lookback];
                let ind2 = self.macd_values[len - 1];

                // Classic bullish divergence: price lower, MACD higher
                if price2 < price1 && ind2 > ind1 {
                    self.value = ((ind2 - ind1).abs()).min(1.0);
                }
                // Classic bearish divergence: price higher, MACD lower
...
mod tests {
    ...
    #[test]
    fn test_macd_divergence_creation() {
        let div = MacdDivergence::new(12, 26, 10);
        assert!(!div.is_ready());
        assert_eq!(div.value().main(), 0.0);
    }

    #[test]
    fn test_macd_divergence_warmup() {
        let mut div = MacdDivergence::new(12, 26, 10);
        for i in 0..60 {
    ...
    #[test]
    fn test_macd_divergence_values_finite() {
        let mut div = MacdDivergence::new(12, 26, 10);
        for i in 0..70 {
