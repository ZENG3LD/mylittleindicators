...
impl RsiDivergence {
    ...
    pub fn update_bar(&mut self, o: f64, h: f64, l: f64, c: f64, v: f64) -> f64 {
        ...
        // Simple divergence check: compare recent swing highs/lows
        if self.prices.len() >= self.lookback {
            let len = self.prices.len();
            let _half = self.lookback / 2;
            ...
            if len >= self.lookback * 2 {
                ...
                // Classic bullish divergence: price lower, RSI higher
                if price2 < price1 && ind2 > ind1 {
                    self.value = ((ind2 - ind1).abs() / 100.0).min(1.0);
                }
                // Classic bearish divergence: price higher, RSI lower
...
mod tests {
    ...
    #[test]
    fn test_rsi_divergence_creation() {
        let div = RsiDivergence::new(14, 10);
        assert!(!div.is_ready());
        assert_eq!(div.value().main(), 0.0);
    }

    #[test]
    fn test_rsi_divergence_warmup() {
        let mut div = RsiDivergence::new(14, 10);
        for i in 0..50 {
    ...
    #[test]
    fn test_rsi_divergence_values_finite() {
        let mut div = RsiDivergence::new(14, 10);
        for i in 0..60 {
