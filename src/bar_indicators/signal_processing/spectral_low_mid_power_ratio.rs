...
mod tests {
    ...
    fn test_spectral_lm_ratio_warmup() {
        ...
        for i in 0..70 {
            ...
            slm.update_bar(price, price + 1.0, price - 1.0, price, 1000.0);
        }
        assert!(slm.is_ready());
    }
    ...
    fn test_spectral_lm_ratio_finite() {
        ...
        for i in 0..100 {
            ...
            let value = slm.update_bar(price, price + 1.0, price - 1.0, price, 1000.0);
    ...
    fn test_spectral_lm_ratio_reset() {
        let mut slm = SpectralLowMidPowerRatio::new(64, 0.15, 0.35);
        for i in 0..70 {
            slm.update_bar(100.0 + i as f64, 101.0, 99.0, 100.0 + i as f64, 1000.0);
        }
        slm.reset();
        assert!(!slm.is_ready());
        assert_eq!(slm.value().main(), 0.0);
